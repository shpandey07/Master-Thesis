...
and to measure the performance of these operations on small-medium sized datasets. It automates the following steps:
...
# Function to load data into Neo4j using APOC and measure performance
def load_data(driver, node_file, label_file, edge_file, num_iterations):
    ...
    for _ in range(num_iterations):
        with driver.session() as session:
            ...
            print("Nodes are loaded!")        

            # Load label data
            label_query = """
            CALL apoc.load.csv($file, {header:true}) YIELD map, lineNo
            WITH map, lineNo - 1 AS row_index
            MATCH (n {idx: row_index})
            SET n.label = map.`0`
            RETURN n
            """       
            ...
            # Load edge data using APOC
            edge_query = """      
            CALL apoc.load.csv($file, {header:true}) YIELD map
            ...
            # Calculate disk space used
            node_space, relationship_space = calculate_disk_space(
                node_count, num_relationships
            )
...
if __name__ == "__main__":
    ...
    node_file = "file:///X_1000_nodes_5_edges.csv"
    edge_file = "file:///edge_index_1000_nodes_5_edges.csv"
    label_file = "file:///y_1000_nodes_5_edges.csv" 
