...
def load_edges(connection, file_path, batch_size=1):
    """
...
def load_nodes(connection, file_path, batch_size=1):
    """
    Loads nodes from a CSV file into the nodes table in the database.
...
def load_labels(connection, file_path, batch_size=1):
    """
...
def update_node_features(connection, num_iterations):
    ...
    logging.info("Node update operation complete!")
...
def add_weight_column(connection):
    ...
    logging.info("Weight column added!")
    cursor = connection.cursor()
    try:
...
def update_edge_weights(connection, num_iterations):
    ...
    logging.info("Edge update operation starting!")
    execution_times = []
    percent = 5
    ...
    logging.info("Edge update operation complete!")
    return mean_time, std_dev_time 
...
if __name__ == "__main__":
    ...
    edge_file = '/home/spandey/Master-Thesis/Code_Final/data/data/syn_data/edge_index_1000000_nodes_5_edges.csv'
    node_file = '/home/spandey/Master-Thesis/Code_Final/data/data/syn_data/X_1000000_nodes_5_edges.csv'
    label_file = '/home/spandey/Master-Thesis/Code_Final/data/data/syn_data/y_1000000_nodes_5_edges.csv'
    ...
    log_file_name = f"postgres_{nodes_count}N_{edges_count}E_{num_iterations}Iterations.log"
    ...
    logging.info(f"Results for dataset_{nodes_count}_nodes_{edges_count}_edges for {num_iterations} iterations in PostgreSQL are:")
    ...
    logging.info(f"Mean Execution Time: {mean_time:.3f} seconds, Std Dev Time: {std_dev_time:.3f} seconds")
    logging.info(f"Mean Space Usage: {mean_space:.3f} MB, Std Dev Space: {std_dev_space:.3f} MB")
    ...
    logging.info(f"Mean time: {mean_time:.3f} seconds, Std Dev time: {std_dev_time:.3f} seconds")
    ...
    logging.info(f"Total Index Creation Time: {total_index_mean_time:.3f} MB, Std Dev Space: {total_index_std_time:.3f} MB")
