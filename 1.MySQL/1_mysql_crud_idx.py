...
and to measure the performance of these operations on small and medium sized datasets. It automates the following steps:
...
log_file_path = "E:/Master Thesis/GitHub_Repo/Master-Thesis/Code_Final/logs/mysql_crud_operations_log_10iterations.txt"
...
def create_tables_with_index(connection):
    ...
    Returns:
        tuple: A tuple containing two values:
            - mean_index_time (float): Mean execution time for creating the indexes.
            - std_dev_index_time (float): Standard deviation of the execution time for creating the indexes.
    """
...
# Function to load edges data into MySQL 
def load_edges(connection, file_path):
    """
    Loads edge data from a CSV file into the 'edges' table in the database.
    ...
    with open(file_path, 'r') as edges_file:
        ...
        for row in edges_reader:
            source, target = row
            cursor.execute("INSERT INTO edges (source, target) VALUES (%s, %s)", (source, target))

        connection.commit()
        print("Edge loading completed!")
        cursor.close()

# Function to load nodes data into MySQL 
def load_nodes(connection, file_path):
    """
    Loads node data from a CSV file into the 'nodes' table in the database.
    ...
    with open(file_path, 'r') as nodes_file:
        ...
        for row in nodes_reader:
            node_data = [float(feature) for feature in row]
...
# Function to load labels data into MySQL 
def load_labels(connection, file_path):
    """
    Loads label data from a CSV file into the 'labels' table in the database.
    ...
    with open(file_path, 'r') as labels_file:
        ...
        for row_id, row in enumerate(labels_reader, start=1):
            label = row[0]
            cursor.execute("INSERT INTO labels (node_id, label) VALUES (%s, %s)", (row_id, label))
...
# Function to delete all tables from MySQL 
...
if __name__ == "__main__":
    ...
    edge_file = 'E:/Master Thesis/Thesis_Code/data/edge_index_10000_nodes_10_edges.csv'
    node_file = 'E:/Master Thesis/Thesis_Code/data/X_10000_nodes_10_edges.csv'
    label_file = 'E:/Master Thesis/Thesis_Code/data/y_10000_nodes_10_edges.csv'
    ...
    # Print results
    print(f"Results for CRUD operation on dataset_{num_nodes}_nodes_{num_edges}_edges in MySQL for {num_iterations} iterations are:")
    ...
    print(f"Mean execution time: {mean_time:.3f} seconds, Std Dev execution time: {std_dev_time:.3f}")
